...
TRANING_CYCLES = 50 # How many epochs to train
BATCH_SIZE = 128
...
if TEST_AFTER_n_EPOCH > TRANING_CYCLES:
    print("WARNING: TEST_AFTER_n_EPOCH is currently bigger than TRANING_CYCLES.")
...
# Training loop ---------------------------------------------------------------
    ...
    def save(model, optimizer, scheduler, epoch, MODEL_ADDRESS): # Saving model function
    ...
    highest_test_accuracy = 0 # For saving model with the highest test accuracy during training
    for epoch in range(epoch+1, epochs+1):
        ...
        if epoch % TEST_AFTER_n_EPOCH == 0:
            ...
            if test_acc > highest_test_accuracy:
                ...
                saved_epoch = epoch
                save(model=God_of_Number, optimizer=optimizer, scheduler=scheduler, epoch=epoch, MODEL_ADDRESS=MODEL_ADDRESS)
